...
// ---------- Validation ----------
if ($full_name === '' || $email === '' || $phone === '' || $district === '' || $village === '') {
...
$stmt = $conn->prepare("INSERT INTO members (registration_id, full_name, email, phone, state, district, village) VALUES (?, ?, ?, ?, ?, ?, ?)");
if (!$stmt) {
    respond('error', 'Database error: ' . $conn->error);
}
$stmt->bind_param('sssssss', $registration_id, $full_name, $email, $phone, $state, $district, $village);
